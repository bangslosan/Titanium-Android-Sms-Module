...
#define TAG "com.hybrid.sms"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Hybdridsms_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Hybdridsms.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::HybdridsmsBindings::lookupGeneratedInit(
...
static void Hybdridsms_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Hybdridsms_getBinding)->GetFunction());
}

static void Hybdridsms_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::HybdridsmsBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry HybdridsmsBinding = {
	"com.hybrid.sms",
	Hybdridsms_init,
	Hybdridsms_dispose
};
...
Java_com_hybrid_sms_HybdridsmsBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.hybrid.sms", &HybdridsmsBinding);
	titanium::KrollBindings::addExternalLookup(&(::HybdridsmsBindings::lookupGeneratedInit));
}
